...
namespace gheith {
    ...
    struct TCBWithStack : public TCB {
        ...
        TCBWithStack(uint32_t pid) : TCB(false, pid) {
            stack[STACK_WORDS - 2] = 0x200;  // EFLAGS: IF
            stack[STACK_WORDS - 1] = (uint32_t) entry;
	        saveArea.no_preempt = 0;
            saveArea.esp = (uint32_t) &stack[STACK_WORDS-2];
    ...
    struct TCBImpl : public TCBWithStack {
        ...
        TCBImpl(T work, uint32_t pid) : TCBWithStack(pid), work(work) {
        }
...
template <typename T>
Shared<gheith::PCB> thread(T work, uint32_t pid, Shared<gheith::PCB> parent) {
    using namespace gheith;

    delete_zombies();
    auto tcb = new TCBImpl<T>(work, pid);
    // deep copy pd and dt
    tcb->pcb->copy_from(parent);
    //tcb->pcb->copy_descriptors(parent)
    schedule(tcb);
    return tcb->pcb;
}
